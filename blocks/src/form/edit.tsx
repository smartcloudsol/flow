...
function blockToFieldConfig(block: FlowBlockInstance): FieldConfig | null {
  ...
  if (
    ...
    const hasOptionData =
      typeof optionsText === "string" ||
      (fieldType === "checkbox-group" &&
        (typeof manualOptionsText === "string" ||
          typeof exclusiveValuesText === "string"));
    ...
    return {
      ...
      options:
        !restAttrs.optionsSource || restAttrs.optionsSource === "static"
          ? parseOptions(typeof optionsText === "string" ? optionsText : "")
          : undefined,
      ...(fieldType === "checkbox-group"
        ? {
            manualOptions: parseOptions(
              typeof manualOptionsText === "string"
                ? manualOptionsText
                : undefined,
            ),
            exclusiveValues: parseTextList(
              typeof exclusiveValuesText === "string"
                ? exclusiveValuesText
                : undefined,
            ),
      ...
      <InspectorControls>
        <PanelBody title={__("Form settings", TEXT_DOMAIN)} initialOpen>
          ...
          <SelectControl
            ...
            onChange={(endpointMethod) =>
              setAttributes({
                endpointMethod:
                  (endpointMethod as "GET" | "POST" | "PUT" | "PATCH") ||
                  "POST",
              })
            }
            help={__(
              "HTTP method used when submitting to a custom endpoint path. GET sends the serialized field values as URL query parameters.",
              TEXT_DOMAIN,
            )}
          />
          <KeyValueListEditor
            label={__("Endpoint headers", TEXT_DOMAIN)}
            value={attributes.endpointHeaders}
            onChange={(endpointHeaders) => setAttributes({ endpointHeaders })}
            keyPlaceholder="X-Request-Source"
            valuePlaceholder="wpsuite"
          />
          <p
            style={{
              fontSize: "12px",
          ...
          >
            {__(
              "Additional headers sent with custom endpoint requests. Header values support the same runtime interpolation tokens as endpoint paths. These are browser-visible request headers, so do not place secrets here.",
              TEXT_DOMAIN,
            )}
          </p>
          <TextControl
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            Object.keys(attributes.colors).length === 0 ? (
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {Object.entries(attributes.colors).map(([key, val], index) => (
                  <li
                    key={index}
                    style={{
                      display: "flex",
                    ...
                    <TextControl
                      ...
                      onChange={(newKey) => {
                        ...
                        const newColors = Object.fromEntries(
                          Object.entries(attributes.colors || {}).map(
                            ([currentKey, currentValue]) => [
                              currentKey === key ? newKey : currentKey,
                              currentValue,
                            ],
                          ),
                        );
