    ...
    <>
      ...
      {(optionsSource === "api" || optionsSource === "autocomplete") && (
        <>
          ...
          />
          <Text size="12px" style={{ marginTop: "-8px", marginBottom: "12px" }}>
            {__(
              "Supports the same runtime interpolation tokens as form endpoints, including {{wpsuite.*}}, {{query.*}}, {{location.*}}, {{wp.*}}, {{global.*}}, and current form field values such as {{email}}.",
              TEXT_DOMAIN,
            )}
          </Text>
          <SelectControl
          ...
          />
          <TextControl
            label={__(`${labelPrefix}Selected Path`, TEXT_DOMAIN)}
            value={value.apiSelectedPath || ""}
            onChange={(apiSelectedPath) => update({ apiSelectedPath })}
            placeholder="status"
            help={__(
              "Optional path inside each result item used to determine whether the option should start selected, e.g. response[].status, selected or checked.",
              TEXT_DOMAIN,
            )}
          />
          <TextControl
            label={__(`${labelPrefix}Selected Value`, TEXT_DOMAIN)}
            value={value.apiSelectedValue || ""}
            onChange={(apiSelectedValue) => update({ apiSelectedValue })}
            placeholder="SUBSCRIBED"
            help={__(
              "Optional comparison value for Selected Path. Leave empty to treat the resolved value as a boolean/truthy flag.",
              TEXT_DOMAIN,
            )}
          />
          <ToggleControl
