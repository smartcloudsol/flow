...
function appendQueryParamValue(
  ...
  if (Array.isArray(value)) {
    value.forEach((entry) => {
      appendQueryParamValue(searchParams, `${key}[]`, entry);
    });
    return;
  }
...
function buildGetRequestPath(
  path: string,
  values: Record<string, unknown>,
): string {
  const [pathWithQuery, hashPart] = path.split("#", 2);
  const [basePath, queryPart] = pathWithQuery.split("?", 2);
  const searchParams = new URLSearchParams(queryPart || "");

  Object.entries(values).forEach(([key, value]) => {
    appendQueryParamValue(searchParams, key, value);
  });
...
function parseCustomEndpointHeaders(
  rawHeaders: unknown,
  wpContext?: FormAttributes["wpContext"],
  ...
  >((headers, [key, value]) => {
    ...
    headers[normalizedKey] = resolveRuntimeContextString(
      normalizedValue,
      wpContext,
      formValues,
    );
...
export function FormShell({
  ...
  const fieldDefaultValuesFromStore = useSelect(
    () =>
      form.formId
        ? getStoreSelect(store).getFormFieldDefaultValues(form.formId)
        : {},
    [store, form.formId],
  );
  ...
  const resolvedEndpointPath = useMemo(() => {
    ...
    const resolvedValue = resolveRuntimeContextUrlString(
      rawValue,
      form.wpContext,
      state.values,
    ).trim();
  ...
  }, [form.endpointPath, form.wpContext, state.values]);
  const resolvedEndpointMethod = useMemo(
    () => form.endpointMethod || "POST",
    [form.endpointMethod],
  );
  const resolvedEndpointHeaders = useMemo(
    () =>
      parseCustomEndpointHeaders(
        form.endpointHeaders,
        form.wpContext,
        state.values,
      ),
    [form.endpointHeaders, form.wpContext, state.values],
  );
  ...
  const emitFormEvent = useCallback(
    (name: string, detail: Record<string, unknown>) => {
      ...
      if (name === "smartcloud-flow:submit-success") {
        hostElement.dispatchEvent(
          new CustomEvent("wps-flow-form:submit-success", {
            detail,
            bubbles: true,
            composed: true,
          }),
  ...
  const dispatchFrontendRequest = useCallback(
    async <TResponse,>(
      path: string,
      body: unknown,
      method: CustomEndpointMethod = "POST",
      extraHeaders: Record<string, string> = {},
    ): Promise<TResponse> => {
      const normalizedPath = path.trim();
      const requestPath =
        method === "GET" && body && typeof body === "object"
          ? buildGetRequestPath(normalizedPath, body as Record<string, unknown>)
          : normalizedPath;
      ...
      const response = await fetch(`${frontendApiBaseUrl}${requestPath}`, {
        method,
        headers: requestHeaders,
        body:
          requestBody === undefined
  ...
  const actions = useMemo(
    () => ({
      ...
      setInitialValue: (name: string, value: unknown) => {
        dispatch({
          type: "RESET_FIELDS",
          values: { [name]: value },
          names: [name],
  ...
  const activeStandaloneHtml = useMemo(
    () =>
      hydrateSuccessStateHtml(activeStandaloneState?.html || "", {
        ...
        formId: form.formId,
        formName: form.formName,
        responseMessage: form.successMessage ?? lastSubmitResponse?.message,
